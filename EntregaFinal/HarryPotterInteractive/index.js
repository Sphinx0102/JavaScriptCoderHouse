...
function createButton(text, id) {
	...
	button.setAttribute("id", `${id}Section`);
...
function createPerfilCard(data) {
	...
	if(data.bando.nombre == 'orden'){
		loreEvent(`Habiendo elegido el bando ${data.bando.datos.titulo}, tomaste partido en la batalla como ${data.personaje.personaje}.
		Dentro del campo de batalla, mientras batallabas contra un enemigo, utilizaste el hechizo ${data.hechizos[3].hechizo}, el cual te ayudo para poder girar las tornas y poder derrotarlo`);
	}
	else{
		loreEvent(`Habiendo elegido el bando ${data.bando.datos.titulo}, tomaste partido en la batalla como ${data.personaje.personaje}.
		Dentro del campo de batalla, mientras batallabas contra un mago, utilizaste el hechizo ${data.hechizos[3].hechizo}, el cual te ayudo para poder ganar ventaja y consecuentemente terminar matando a dicho mago `);
	}
